import csv, json, os, re, time, pathlib, html as htmlmod
LIMIT=int(os.environ.get("TEST_LIMIT","0"))
IN,OUT,STATE="urls.csv","produits_nutrition.csv","state.json"
FIELDS=["proteines","matieres_grasses","cellulose","cendres","humidite","calcium","phosphore","sodium","omega_3","omega_6","calories"]
LABELS={"proteines":r"prot[eé]ines?(?:\s*brutes?)?","matieres_grasses":r"(?:mati[eè]res?\s+grasses|lipides)","cellulose":r"(?:cellulose|fibres?)(?:\s*brutes?)?","cendres":r"cendres(?:\s*brutes?)?","humidite":r"humidit[eé]","calcium":r"calcium","phosphore":r"phosphore","sodium":r"sodium","omega_3":r"om[eé]ga.?3","omega_6":r"om[eé]ga.?6","calories":r"kcal"}
def load_state():
...
def extract(raw):
    ...
    for f,pat in LABELS.items():
        m=re.search(r"(?:"+pat+r")\s*[:=]?\s*(\d+[.,]?\d*)\s*%",t,re.I)
